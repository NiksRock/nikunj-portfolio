  ...
  return (
    <Reveal delay={0}>
      <div
        ...
        style={{ border: '1px solid rgba(42,58,80,.5)', transition: 'transform .25s, box-shadow .25s, border-color .25s', cursor: 'pointer' }}
        onMouseEnter={(e) => {
          sfxCard();
      ...
      >
        ...
        <div style={{ height: 165, background: project.bg, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'flex-end', padding: '14px 18px' }}>
        ...
        <div style={{ padding: '18px 18px 14px' }}>
          <SpeakText style={{ fontSize: 13, color: 'rgba(232,240,255,.58)', lineHeight: 1.6, marginBottom: 14, display: 'block' }}>
            {project.desc}
          </SpeakText>
          ...
          <div style={{ display: 'flex', gap: 0, marginBottom: 14, borderTop: '1px solid rgba(42,58,80,.4)', borderBottom: '1px solid rgba(42,58,80,.4)', padding: '9px 0' }}>
            {project.metrics.map((metric, i) => (
              <div
                key={metric.l}
                style={{ flex: 1, textAlign: 'center', borderRight: i < project.metrics.length - 1 ? '1px solid rgba(42,58,80,.4)' : 'none' }}
              >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
            {project.tags.map((tag) => (
              <span key={tag} style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 9, color: 'var(--muted)', border: '1px solid rgba(42,58,80,.5)', padding: '2px 7px' }}>
                {tag}
              </span>
            ))}
          </div>
...
function ProjectModal({ project, onClose }) {
  ...
  return (
    <div
      className="modal-bg"
      style={{ position: 'fixed', inset: 0, background: 'rgba(2,4,8,.88)', backdropFilter: 'blur(10px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      onClick={() => { sfxClick(); onClose(); }}
    >
      <div
        className="card"
        onClick={(e) => e.stopPropagation()}
        style={{ maxWidth: 610, width: '100%', border: `1px solid ${project.color}55`, boxShadow: `0 0 60px ${project.color}18`, maxHeight: '90vh', overflowY: 'auto' }}
      >
        <Brackets color={project.color} />

        {/* Modal header */}
        <div style={{ height: 105, background: project.bg, position: 'relative', padding: '16px 24px', display: 'flex', alignItems: 'flex-end' }}>
          ...
          <button
            onClick={() => { sfxClick(); onClose(); }}
            style={{ position: 'absolute', top: 12, right: 12, background: 'none', border: '1px solid rgba(255,70,85,.4)', color: 'var(--red)', width: 28, height: 28, cursor: 'pointer', fontFamily: "'Share Tech Mono',monospace", fontSize: 13, zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            ✕
          </button>
        </div>
        ...
        <div style={{ padding: '22px 24px' }}>
          ...
          <SpeakText style={{ fontSize: 14, lineHeight: 1.7, color: 'rgba(232,240,255,.72)', marginBottom: 22, display: 'block' }}>
            {project.details}
          </SpeakText>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 22 }}>
            {project.metrics.map((metric) => (
              <div key={metric.l} style={{ background: 'rgba(2,4,8,.6)', padding: 14, textAlign: 'center', border: `1px solid ${project.color}28` }}>
                <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 26, color: project.color }}>{metric.v}</div>
                <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 9, color: 'var(--muted)' }}>{metric.l}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
            {project.tags.map((tag) => (
              <span key={tag} style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 10, color: 'var(--white)', background: 'rgba(42,58,80,.4)', border: '1px solid rgba(42,58,80,.7)', padding: '4px 12px', clipPath: 'var(--clip-sm)' }}>
                {tag}
              </span>
            ))}
          </div>
